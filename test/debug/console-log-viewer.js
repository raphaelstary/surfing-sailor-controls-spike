...
var ConsoleLogViewer = (function () {
    ...
    ConsoleLogViewer.prototype.getFormattedTime = function () {
        var date = new Date();
        return this.format(date.getHours(), 2) + ":" + this.format(date.getMinutes(), 2) + ":" +
            this.format(date.getSeconds(), 2) + ": " + this.format(date.getMilliseconds(), 3);
    }

    ConsoleLogViewer.prototype.format = function (v, x) {
        if (x == 2) return (v < 10) ? "0" + v : "" + v; else if (x == 3) {
            if (v < 10) return "00" + v; else if (v < 100) return "0" + v; else return "" + v;
        }
    }
    ...
    ConsoleLogViewer.prototype.log = function (args, color, splitArgs) {
        ...
        _items.push(
            "<font class='log-date'>" + this.getFormattedTime() + "</font> &nbsp; <font class='" + color + "'>" +
            (splitArgs ? Array.prototype.slice.call(args).join(",") : args) + "<\/font>");
    ...
    ConsoleLogViewer.prototype.overwrite = function () {
        ...
        window.onerror = function (message, file, lineNumber) {
            self.log(arguments, "log-error", true);
            if (original.window.onerror) return original.window.onerror(message, file, lineNumber); else return false;
        }
    }

    ConsoleLogViewer.prototype.addDivs = function (self) {
        var alignment = window.location.href.indexOf("console_at_bottom=true") > -1 ||
        window.location.href.indexOf("console_at_bottom=1") > -1 ? "bottom-aligned" : "top-aligned";
        ...
        document.getElementById("debug_console_position_button").addEventListener("click", function (e) {
            document.getElementById("debug_console").className = (document.getElementById("debug_console").className ==
            "top-aligned") ? "bottom-aligned" : "top-aligned";
            e.preventDefault();
        }, false);
